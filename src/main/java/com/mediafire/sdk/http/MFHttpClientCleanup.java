...
public final class MFHttpClientCleanup extends MFHttp {
    ...
    public MFHttpClientCleanup(MFConfiguration mfConfiguration, MFTokenFarmInterface mfTokenFarmCallback) {
        super(mfConfiguration);
        this.mfTokenFarmCallback = mfTokenFarmCallback;
    }

    /**
     * returns a session token to the MFTokenFarmInterface.
    ...
    public void returnToken(MFRequester mfRequester, MFResponse mfResponse) throws MFHttpException {
        mfConfiguration.getMFLogger().d(TAG, "returnToken(type: " + mfRequester.getTypeOfTokenToReturn().toString() + ")");
        ...
        switch (mfRequester.getTypeOfTokenToReturn()) {
            ...
            case V2:
                ...
                if (apiResponse.hasError() && apiResponse.getError() == 105 || apiResponse.getError() == 127) {
                    mfConfiguration.getMFLogger().w(TAG, "not session token was spoiled for request: " + mfRequester.getUri() + mfRequester.getRequestParameters().toString() + " using session token: " + mfRequester.getToken().toString());
                    mfTokenFarmCallback.sessionTokenSpoiled((MFSessionToken) mfResponse.getOriginMFRequester().getToken());
                } else {
                    if (apiResponse.needNewKey()) {
                        mfConfiguration.getMFLogger().v(TAG, "original session token borrowed for request " + mfRequester.getUri() + mfRequester.getRequestParameters().toString() + ": " + mfRequester.getToken().toString());
                        ((MFSessionToken) mfRequester.getToken()).updateSessionToken();
                        mfConfiguration.getMFLogger().v(TAG, "returning updated session token: " + mfRequester.getToken().toString() + " for request: " + mfRequester.getUri() + mfRequester.getRequestParameters().toString());
                    }
            ...
            case UPLOAD:
                ...
                if (uploadActionTokenResponse.hasError() && uploadActionTokenResponse.getError() == 105 || uploadActionTokenResponse.getError() == 127) {
                    ...
                    mfConfiguration.getMFLogger().v(TAG, "returning new upload action token: " + mfUploadActionToken.toString());
            ...
            case IMAGE:
                ...
                if (imageActionTokenResponse.hasError() && imageActionTokenResponse.getError() == 105 || imageActionTokenResponse.getError() == 127) {
                    ...
                    mfConfiguration.getMFLogger().v(TAG, "returning new image action token: " + mfImageActionToken.toString());
            ...
            case NONE:
                ...
                if (!mfRequester.getUri().contains("get_session_token")) {
                    mfConfiguration.getMFLogger().v(TAG, "not returning a token for request: " + mfRequester.getUri() + mfRequester.getRequestParameters().toString());
                } else {
                    mfConfiguration.getMFLogger().v(TAG, "not returning a token for request: " + mfRequester.getUri());
                }
                ...
                if (mfResponse.getResponseObject(ApiResponse.class).hasError()) {
                    mfConfiguration.getMFLogger().w(TAG, "api response had error: " + mfResponse.getResponseObject(ApiResponse.class).getErrorCode());
    ...
    private MFActionToken createActionToken(MFActionToken.Type type, GetActionTokenResponse getActionTokenResponse, MFRequester mfRequester) {
        mfConfiguration.getMFLogger().d(TAG, "createActionToken()");
        if (getActionTokenResponse == null) {
            mfConfiguration.getMFLogger().v(TAG, "response was null, returning null");
            return null;
        }

        if (getActionTokenResponse.hasError()) {
            mfConfiguration.getMFLogger().v(TAG, "response had error, returning null");
            return null;
        }
        ...
        mfConfiguration.getMFLogger().v(TAG, "created new upload action token: " + actionToken.toString());
        return actionToken;
    }

    private MFSessionToken createNewSessionToken(GetSessionTokenResponse getSessionTokenResponse) throws MFHttpException {
        mfConfiguration.getMFLogger().d(TAG, "createNewSessionToken()");
        ...
        if (getSessionTokenResponse.hasError()) {
            mfConfiguration.getMFLogger().v(TAG, "response had error, returning error");
        ...
        mfConfiguration.getMFLogger().v(TAG, "created new token: " + mfSessionToken.toString());
