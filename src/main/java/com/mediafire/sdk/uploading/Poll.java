...
class Poll extends UploadRunnable {
    ...
    public void run() {
        if (isDebugging()) {
            System.out.println("starting upload/poll_upload for " + mUpload.getFile());
        }
        ...
        while (pollCount < mMaxPolls) {
            ...
            if (!resultValid(result)) {
                if (isDebugging()) {
                    System.out.println("cancelling upload/poll_upload for " + mUpload.getFile() + " due to invalid result object");
                }
            ...
            try {
                ...
                if (isDebugging()) {
                    System.out.println("cancelling upload/poll_upload for " + mUpload.getFile() + " due to an exception: " + exception);
                }
            ...
            if (apiResponse == null) {
                if (isDebugging()) {
                    System.out.println("cancelling upload/poll_upload for " + mUpload.getFile() + " due to a null ApiResponse object");
                }
                mProcessMonitor.generalCancel(mUpload, result);
                return;
            }
            ...
            if (apiResponse.hasError()) {
                if (isDebugging()) {
                    System.out.println("cancelling upload/poll_upload for " + mUpload.getFile() + " due to an ApiResponse error (" + apiResponse.getMessage() + ", error " + apiResponse.getError() + ")");
                }
            ...
            if (doUpload.getFileErrorCode() != 0) {
                if (isDebugging()) {
                    System.out.println("cancelling upload/poll_upload for " + mUpload.getFile() + " due to a file error code (" + doUpload.getFileErrorCode() + ") ApiResponse error (" + apiResponse.getMessage() + ", error " + apiResponse.getError() + ")");
                }
                mProcessMonitor.apiError(mUpload, result);
                return;
            }
            ...
            if (doUpload.getQuickKey() != null && !doUpload.getQuickKey().isEmpty()) {
                if (isDebugging()) {
                    System.out.println("upload/poll_upload for " + mUpload.getFile() + " has finished and received quick key " + doUpload.getQuickKey());
                }
            ...
            try {
                if (isDebugging()) {
                    System.out.println("pausing upload/poll_upload for " + mUpload.getFile() + " before next poll for " + mWaitTimeBetweenPollsMillis + "ms");
                }
        ...
        if (isDebugging()) {
            System.out.println("max poll attempts (" + mMaxPolls + ") have been reached with upload/poll_upload for " + mUpload.getFile());
        }
